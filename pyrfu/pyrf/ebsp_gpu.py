...
def ebsp_gpu(e_xyz, db_xyz, b_xyz, b_bgd, xyz, freq_int, **kwargs):
    ...
    res['bb_xxyyzzss'] =  res['bb_xxyyzzss'].data
    res['ee_xxyyzzss'] =  res['ee_xxyyzzss'].data
    res['pf_xyz'] = res['pf_xyz'].data
    for key, value in res.items():
        if isinstance(res[key], xr.DataArray):
            res[key] = res[key].data

    for key, value in res.items():
        #if isinstance(value, xr.DataArray):
        #    res[key] = value.data  # Extract raw data and retain dimensions
        if hasattr(value, "shape"):
            if value.ndim == 2:
                if value.shape == (9598, 32):  # Time-Frequency Data
                    res[key] = (("time", "frequency"), value)
                elif value.shape == (32, 9598):  # If the shape is reversed
                    res[key] = (("frequency", "time"), value)
                elif value.shape == (1536, 3) :
                    res[key] = (("v1", "v2"), value)
                elif value.shape == (7, 3) :
                    res[key] = (("v3", "v2"), value)
            elif value.ndim == 3:
                if value.shape[2] == 3:  # (9598, 32, 3) - Likely XYZ components
                    res[key] = (("time", "frequency", "vector"), value)
                elif value.shape[2] == 4:  # (9598, 32, 4) - Tensor components?
                    res[key] = (("time", "frequency", "tensor"), value)
                elif value.shape[2] == 2:  # (9598, 32, 4) - Tensor components?
                    res[key] = (("time", "frequency", "2Dvector"), value)
